...
def load_data(simulation_path: Path, observation_path: Path = None, index_column: str = "date", observation_column: str = "obs", simulation_column: str = "sim") -> dict[str, pd.DataFrame]:
    data = {}
    ...
    if obs_files:
        ...
        obs_data = {file.stem: pd.read_csv(file, parse_dates=[index_column]) for file in obs_files}
        sim_data = {file.stem: pd.read_csv(file, parse_dates=[index_column]) for file in sim_files}
...
def generate_matplotlib_plot(catchment, df, index_column="date", observation_column="obs", simulation_column="sim"):
